...
export default function AyahCard({ ayah, surahName, surahNumber }) {
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        ...
        text: `${ayah.text}\n${ayah.translation}`,
        url: window.location.href + '#ayah-' + ayah.numberInSurah,
      });
    }
  };
  ...
  return (
    <div 
      ...
      className="p-8 bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow group"
    >
